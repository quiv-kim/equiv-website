(() => {
  ...
  const renderReadinessResult = () => {
    if (!readinessSteps) return;
    const result = readinessResults[getReadinessLevel()];
  ...
  if (!readinessOpen || !readinessModal || !readinessSteps) return;

  readinessOpen.addEventListener("click", openReadinessModal);
  readinessCloseItems.forEach((item) => item.addEventListener("click", closeReadinessModal));
