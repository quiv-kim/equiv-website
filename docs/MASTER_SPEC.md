...
Mobile Typography는 HOME Hero Title 28px을 사용하고, Main Section Title과 모든 Detail Page H1은 동일한 22px을 사용한다. Card Title은 16px, Body와 Button은 14px, Auxiliary Text와 Mobile Navigation은 13px로 통일한다. Form Input은 iOS 자동 확대 방지를 위해 16px을 유지한다. Desktop과 Tablet에서도 Detail Page H1과 주요 Section Title은 HOME Main Section의 `clamp(30px, 4vw, 52px)` Scale을 공유한다.
