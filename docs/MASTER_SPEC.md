...
4. What We Review
5. EQUIV Value Framework
6. Strategic Value Analysis
7. Case Perspective
8. FAQ
9. Brand Statement
10. CTA
...
- Header Wordmark는 공백 없는 단일 텍스트 `EQUIV`로 작성하며 자간은 `-0.01em`을 기준으로 한다. 글자별 Span이나 Margin으로 자간을 만들지 않는다.
- Header Lockup은 Desktop `Mark 30px / Divider 26px / Wordmark 23px / Gap 10px`, Tablet `28px / 24px / 22px / 9px`, Mobile `26px / 22px / 19px / 8px`을 공통 기준으로 사용한다.
...
Mobile Typography는 HOME Hero Title 28px과 Main Section Title 22px을 사용한다. Service·Business Valuation 상세 H1은 HOME Service Card의 실제 `16px / 1.5`, Insights 상세 H1은 HOME Insights Card의 실제 `16px / 1.5`를 그대로 사용한다. Desktop에서는 각각 HOME Component의 실제 값인 Service `32px / 1.28`, Insights `21px / 1.45`를 사용한다. 모든 상세 CONTACT Title은 HOME Contact의 Mobile `22px / 1.36` 규칙을 공유한다. Body와 Button은 14px, Auxiliary Text와 Mobile Navigation은 13px로 통일하며 Form Input은 iOS 자동 확대 방지를 위해 16px을 유지한다.
