...
Mobile Typography는 LISTING 모바일의 여백 우선 Premium Corporate 밀도를 Benchmark로 사용한다. 모든 Component는 공통 Mobile Device Token을 사용하며 페이지별 수치를 별도로 선언하지 않는다. iOS 자동 확대 방지를 위한 Form Input은 Mobile H2 Token인 16px을 사용한다. Section Padding은 유지하고 HOME Hero 높이만 Mobile에서 `80svh`로 조정한다.
