...
Featured Expertise의 대표 제목은 `EQUIV(이퀴브) 전문 서비스`로 운영하고 별도의 설명 문구를 두지 않는다. 카드는 전체 콘텐츠를 수직 중앙에 모은 짧은 선택 메뉴로 구성하며, 영문 부제와 CTA 사이의 간격은 약 10px, 한글 제목에서 CTA까지의 전체 시각적 간격은 약 30~40px로 유지한다.
...
#### Service Detail Copy Principle

Service Page의 Hero는 감성적 슬로건보다 서비스의 대상, 목적과 지원 범위를 한 문장으로 설명한다. `좋은 매각 / 좋은 인수 / 좋은 투자유치`처럼 결과를 추상적으로 평가하는 표현은 사용하지 않는다. `좋은 거래는 올바른 원칙에서 시작됩니다.`는 HOME Principle의 고유 문구로만 운영한다.

Overview는 고객이 어떤 상황에서 서비스를 필요로 하는지와 EQUIV가 어디까지 지원하는지를 두 개의 짧은 문단으로 설명한다. How We Work는 기업가치 검토, 자료 작성, 후보 발굴, 의향 확인, 협상, 실사, 계약과 실행계획 등 서비스별 실제 업무를 네 단계 안에서 구체적으로 보여준다.

CTA는 철학이나 브랜드를 반복하지 않고 해당 서비스의 검토 목적, 상담에서 확인할 내용과 다음 행동을 직접 안내한다. 서비스별 문장 종결은 `설계합니다 / 지원합니다 / 검토합니다 / 구체화합니다 / 안내합니다 / 수행합니다`처럼 역할에 맞게 분산한다.
...
Hero의 공개 진단 영역은 `기업가치 사전진단 / 내 회사의 기업가치는?`를 한글 중심으로 표시한다. 핵심 재무정보와 사업 특성을 기반으로 현재 기업가치의 참고 범위를 확인하는 서비스임을 안내하고, 입력정보가 저장되거나 전송되지 않는다는 원칙과 `예상 소요시간 5분 내외`를 함께 표시한다. 별도 카드, 테두리나 강한 그림자를 사용하지 않고 Hero 배경에 자연스럽게 통합하며 골드는 실행 버튼에만 절제하여 사용한다.
...
페이지 제목은 `기업 매각`, 영문 제목은 `Sell-side M&A`를 사용한다. How We Work는 `기업 및 매각조건 검토 / 기업가치 및 자료 준비 / 후보 발굴 및 의향 확인 / 협상·실사·거래 종결`의 네 단계로 구성하고, 각 단계는 실제 진행 업무를 한 문장으로 설명한다.
...
## Typography Design System Principle

EQUIV의 모든 공개 페이지와 공통 Component는 하나의 Typography Design System을 사용한다. Device Token은 Desktop과 Mobile 각각 `Display / H1 / H2 / H3 / H4 / Body Large / Body / Small / Caption`의 9단계로 관리한다.

CSS 선택자에서 `font-size` 숫자를 직접 선언하지 않는다. 모든 Typography는 `--desktop-*`, `--mobile-*` Device Token에 연결된 공통 `--type-*` Semantic Token을 사용한다. 향후 글자 크기 변경은 Token 값만 수정하고 페이지별 예외 Scale을 추가하지 않는다.
...
기본 Spacing은 Desktop `4 / 8 / 12 / 16 / 24 / 32 / 48px`, Mobile `4 / 8 / 10 / 14 / 20 / 28 / 40px` Scale을 사용한다. Section은 별도의 의미 기반 `Large / Medium / Small` Token으로 관리하며 Desktop은 `96 / 72 / 56px`, Mobile은 `64 / 52 / 40px`을 사용한다.
...
Typography 역할은 `Hero Main Title = Display`, `Page Hero = H1`, `Section Title = H2`, `Card Title = H3`, `Small Card Heading = H4`, `Lead = Body Large`, `Paragraph = Body`, `Label = Small`, `Caption = Caption`으로 고정한다. 카드 제목이나 Form Label에 Section H2 역할을 적용하지 않는다.

Card는 Regular와 Compact Padding을 구분하고, 반복되는 본문 폭은 `Content Width SM / MD / LG / XL` Token으로 관리한다. CSS 후반에는 Mobile·Desktop 역할별 Typography Rule과 Shared Component Rule을 각각 하나의 명확한 영역에 유지하며, 앞선 규칙을 무효화하는 Legacy Override를 추가하지 않는다.
...
Mobile Typography는 LISTING 모바일의 여백 우선 Premium Corporate 밀도를 Benchmark로 사용한다. 모든 Component는 공통 Mobile Device Token을 사용하며 페이지별 수치를 별도로 선언하지 않는다. iOS 자동 확대 방지를 위한 Form Input은 Mobile H4 Token인 16px을 사용한다. HOME Hero 높이는 Mobile에서 `80svh`로 유지한다.
