...
HOME Hero 상단은 `EQUIV (이퀴브) │ M&A 중개·자문 브랜드`, 메인 제목은 `기업의 가치를 이해합니다.`, Subtitle은 `가장 적합한 상대를 찾아 성공적인 M&A를 완성합니다.`로 운영한다. 별도의 회사소개나 장문의 설명 문단을 두지 않으며 CTA는 `기업 매각 상담`을 Primary, `기업 인수 상담`을 Secondary로 유지한다.
