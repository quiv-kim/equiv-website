...
- 숫자는 아라비아 숫자를 사용한다.
- 기간은 실제 운영 기준이 확인된 경우에만 구체적으로 표기한다.
- 비율: `15%`
...
`기업의 가치`와 `적합한 상대`는 Headline과 Subtitle에서 한 번씩 연결한다. `성공적인 M&A`는 Home Hero의 브랜드 핵심 카피로 제한적으로 유지하며 결과 보장 문구로 다른 Page에 반복하지 않는다.
...
> 상담 신청이 정상적으로 접수되었습니다. 담당자가 내용을 확인한 후 연락드리겠습니다. 필요한 자료는 상담 과정에서 별도로 안내드리며, 상담 내용은 비밀유지 원칙에 따라 안전하게 관리됩니다.
...
- `og:site_name`은 공식 브랜드명 `EQUIV` 사용
- `og:image`는 외부에서 접근 가능한 실제 이미지 URL 사용
- canonical과 `og:url`은 확인된 운영 도메인의 절대 URL만 사용
- 운영 도메인이 확정되지 않은 경우 Placeholder URL을 넣지 않는다.
